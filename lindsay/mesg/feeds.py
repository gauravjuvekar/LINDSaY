...
class SubDivisionFeed(Feed):
    def get_object(self, request, division_name, subdivision_name):
        return get_object_or_404(Category, name=subdivision_name,
    ...
    def link(self, subdivision):
        return reverse('mesg:subdivision',
                       kwargs={'division_name': subdivision.parent.name,
                               'subdivision_name': subdivision.name})

    def items(self, subdivision):
        messages = get_messages(subdivision,
                                Q(expires_date__isnull=True) |
                                Q(expires_date__gte=timezone.now())
                                ).order_by('-pub_date')
        return messages
